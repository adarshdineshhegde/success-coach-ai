...
from memory.signals import get_all_signals
...
st.caption("Flagged signals across all students, sorted by severity and urgency.")
...
if not all_signals:
    st.info("No signals flagged yet. Signals appear here after sessions end.")
    st.stop()
...
today  = sum(1 for s in all_signals if s["urgency"] == "today")
...
if not all_signals:
    st.info(f"No {severity_filter.lower()} severity signals.")
    st.stop()

# signal cards
for sig in all_signals:
    icon    = SEVERITY_COLOR.get(sig["severity"], "🟡")
    urgency = URGENCY_LABEL.get(sig["urgency"], sig["urgency"])

    with st.expander(
        f"{icon} {sig['student_name']}  ·  {urgency}  ·  {sig['session_date']}"
    ):
        st.markdown(f"**Concern:** {sig['concern']}")
        st.caption(
